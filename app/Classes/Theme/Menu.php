...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        {
            try {
                if(!auth()->user()->hasPermissionTo($item['auth']))
                    return "";
            }catch (\Exception $e){
                return "";
            }
        ...
        if (isset($item['separator'])) {
            ...
            if (isset($item['parent'])) {
                ...
                if (isset($item['page'])) {
                    if(isset($item['isRoute']) && $item['isRoute'] == true) {
                        if (isset($item['parameters']))
                            $url = route($item['page'], $item['parameters']);
                        else
                            $url = route($item['page']);
                    } else {
                        $url = url($item['page']);
    ...
    public static function isActiveVerMenuItem($item, $page, $rec = 0)
    {
        ...
        if (isset($item['page'])) {
            if(isset($item['isRoute']) && $item['isRoute'] == true) {
                if (isset($item['parameters']))
                {
                    foreach ($item['parameters'] as $pageKey => $pageValue)
                    {
                        if (Route::current()->hasParameter($pageKey)
                            && Route::current()->parameter($pageKey) === $pageValue
                            && Route::current()->getAction('as') === $item['page'])
                            return true;
                    }
                }
                else
                {
                    if (Route::current()->uri() === $page && Route::current()->getAction('as') === $item['page'])
                        return true;
                }
            } else {
                if ($item['page'] === $page)
